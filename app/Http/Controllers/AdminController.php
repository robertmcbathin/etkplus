...
public function postLoadGallery(Request $request){
  ...
  return redirect()->back();
    ...
    public function postToggleShopFunctionPartner(Request $request){
      $partner_id = $request->partner_id;
      $is_shop = $request->is_shop;
      ...
      if ($is_shop == 1) {
        ...
        Session::flash('success','Функционал магазина успешно включен');
        return redirect()->back();
      } else{
        try {
          DB::table('ETKPLUS_PARTNERS')
        ...
        Session::flash('success','Функционал магазина отключен');
        return redirect()->back();
      }
    }
    /**
...
public function getShowShopProduct($product_id = null){
  ...
  ->get();
    ...
    public function postAddShopProductSecondaryImages(Request $request){
      $productId = $request->product_id;
      /**
      SECONDARY_IMAGES
      **/
      ...
      Session::flash('success','Изображения добавлены');
      return redirect()->back();
      }

    }

    public function postEditShopGalleryItem(Request $request){
      $gallery_item_id = $request->gallery_item_id;
      $image_caption = $request->image_caption;
      DB::table('ETKTRADE_PRODUCT_PHOTOS')
      ->where('id', $gallery_item_id)
    ...
    public function postDeleteShopGalleryItem(Request $request){
      $gallery_item_id = $request->gallery_item_id;
      $gallery_item_path = $request->image_path;
      DB::table('ETKTRADE_PRODUCT_PHOTOS')
      ->where('id', $gallery_item_id)
